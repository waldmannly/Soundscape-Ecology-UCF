...
multiple_sounds_EditforLS <- function(directoryArr,wave_FilesArr, resultfile, soundindex = c("ndsi", "acoustic_complexity", "acoustic_diversity", "acoustic_evenness", "bioacoustic_index", "H"), no_cores = 1, fromArr, toArr, units = "seconds", ...){
  ...
  if (no_cores>1){
    ...
    for (soundfile in wav_files){
      this_res <- getfunctionforcluster(soundfile, ...)
